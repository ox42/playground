...
export default {
  ...
  methods: {
    ...
    async listenIframe({ data = {} }) {
      ...
      } else if (data.type === 'cloudapps-console') {
        if (data.method === 'clear') {
          this.clearLogs()
        } else {
          this.addLog({ type: data.method, message: data.args.join('') })
        }
      } else if (data.type === 'cloudapps-make-output-active') {
        this.setActivePan('output')
      } else if (data.type === 'cloudapps-set-boilerplate' && data.boilerplate) {
    ...
    async run() {
      ...
      try {
        ...
        js = `
          ...
          console.clear();
          document.addEventListener('DOMContentLoaded', __executeCloudApps);
          function __executeCloudApps(){
      ...
      const cloudappsRuntime = createElement('script')(`
        ...
        if (shouldUpdateGist) {
          ...
          if (token) {
            ...
            axios(`https://api.github.com/gists/${data.id}`, {
              ...
              params,
              data: {
                description: `Try it online! https://play.cloudapps.io/gist/${
                  data.id
                }`
              }
