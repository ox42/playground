...
const store = new Vuex.Store({
  state: {
    ...
    activePan: 'js',
    autoRun: false,
    githubToken: localStorage.getItem('cloudapps:gh-token') || '',
    gistMeta: {},
    userMeta: JSON.parse(localStorage.getItem('cloudapps:user-meta')) || {},
    editorStatus: 'saved',
    iframeStatus: null,
    transforming: false
  },
  ...
  actions: {
    ...
    async setGist({ commit, dispatch, state }, id) {
      ...
      const main = {
        ...
        ...(files['cloudapps.js'] ? req(files['cloudapps.js'].content) : {}),
        ...(files['cloudapps.json'] ? JSON.parse(files['cloudapps.json'].content) : {})
      }
      for (const type of ['html', 'js', 'css']) {
        if (!main[type].code) {
    ...
    async setGitHubToken({ commit, dispatch }, token) {
      ...
      let userMeta = {}
      if (token) {
        localStorage.setItem('cloudapps:gh-token', token)
        userMeta = await api('user', token)
      } else {
        localStorage.removeItem('cloudapps:gh-token')
      }
      commit('SET_USER_META', userMeta)
      if (Object.keys(userMeta).length > 0) {
        localStorage.setItem('cloudapps:user-meta', JSON.stringify(userMeta))
      } else {
        localStorage.removeItem('cloudapps:user-meta')
      }
    },
    editorSaved({ commit }) {
